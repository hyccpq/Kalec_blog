...
import webpackConfig from '../build/webpack.config.manage.cjs'
...
export const webpackDev = async app => {
  ...
  app.use(async (ctx, next) => {
    try {
      ...
      ctx.response.body = devMiddleware.devMiddleware.fileSystem.createReadStream(
        filename
      )
      ...
      ctx.render('error.ejs', {
        code: error.code,
        info:
          '这是管理端开发环境，页面渲染出错或不存在，点击下方按钮调整管理页面',
        title: '出现错误',
        jumpUrl: '/'
      })
    }
