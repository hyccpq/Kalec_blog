...
import {
    ...
    updateShowGallery,
    getQiniuToken,
    getOneGalleryImages, delSelectImage, editSelectImage, setCoverImage
} from '../service/getAdminGallery'
...
class AdminGalleryApi {
    ...
    async editImages(ctx, next) {
        try {
            ...
            let data = await editSelectImage(id, imageId, {imageDesc, imageName, show})
            ctx.body = resData(1, '查询成功', data)
        } catch (e) {
            ctx.body = resData(0, '出现错误', e.toString())
        }
