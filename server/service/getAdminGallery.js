...
import { qiniu as qiniuSecretConf } from '../conf/userConf.js'
...
const QINIU_UPDATE_OPTION = {
  scope: BUCKET,
  returnBody:
    '{"key":"$(key)","hash":"$(etag)","fsize":$(fsize),"name":"$(x:name)","type":"$(mimeType)"}'
}
...
export const saveNewGallery = async (
  title,
  author,
  description,
  password,
  url = 'http://img.kalecgos.top',
  isPwd = false
) => {
  let query = {
    title,
...
export const savedGallery = async (
  id,
  title,
  author,
  description,
...
export const getAllGallery = async (
  ...
  try {
    if (isManage) {
      let data = await GalleryItem.find(
        {},
        'title author description url coverImgPath show pv like meta password'
      )
        .sort({
          ['meta.updateTime']: -1
        })
      ...
      const data = await GalleryItem.find(
        querys,
        'title author description url coverImgPath show pv like meta'
      )
  ...
  try {
    ...
    if (imageData) {
      if (imageData.images.length)
        await FileManage.instance.deleteListFile(
          imageData.images.map(item => item.imagePath),
          BUCKET
        )
...
export const setCoverImage = async (id, imagePath) => {
  try {
    let data = await GalleryItem.findByIdAndUpdate(id, {
...
export const editSelectImage = async (
  id,
  imageId,
  { imageName, imageDesc, show }
) => {
  try {
    let galleryItem = await GalleryItem.findById(id)
