<template>
    <div id="comment">
        ...
        <Form ref="commentInformation" :model="commentInformation" :label-width="40">
            ...
            <FormItem>
                <Button type="primary" size="default" @click="submitComment">提交</Button>
            </FormItem>
        </Form>
    </div>
</template>
...
<script>
	import { Input,Form,FormItem,Row,Col } from 'iview'
	export default {
		name: "comment",
		components:{
			iInput:Input,
		...
		data(){
			...
			return {
				commentInformation:{
					_id:'',
					user:localStorage.getItem('admin')==='hyccpq'?'Kalecgos':sessionStorage.getItem('admin'),
		...
		methods:{
			...
			// },
			// getCaptcha(){
			//   this.axios.get('/admin/v0/getCaptcha')
			//     .then(res => {
